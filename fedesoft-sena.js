...
function zombie(gameObject) {
    ...
    console.log("Te has encontrado con Zombie")

    if(gameObject.items.ecto > 0){
        gameObject.items.ecto--;
        gameObject.stats.fuerza += 5;
        console.log("Zombie fuer eliminado con ecto");
        console.log("Ecto restantes : " + gameObject.items.ecto)
        return gameObject;
    }

    console.log("Zombie ataco");

    if (gameObject.stats.fuerza <= 0){
        gameObject.stats.carisma -= 2;
        gameObject.stats.fuerza -= 2;
        gameObject.stats.estilo -= 2;
